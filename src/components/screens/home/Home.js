...
const Home = ({route, navigation}) => {
 ...
 useEffect(() => {
  ...
  const renderItem = ({item}) => {
    ...
    const color ='white';
    return (
      <Item item={item} onPress={() => { setSelectedId([item.id,item.familyName]); getDataFromDB(item.id);}} 
        backgroundColor={backgroundColor} textColor={color} opacity={opacity}/>
    );
  };
...
const adProduct = (data) => {
  ...
  const ProductCard = ({data})  => {
    return (
        <TouchableOpacity onPress={()=> navigation.navigate("ProductInfo", {productID:data})}
          style={{ width:'48%', marginVertical: 8 }}>
          <View style={styles.pc_container1}>
            <View style={styles.pc_container2}>
              <Text style={{ fontSize: 12, color: COLOURS.white, fontWeight: 'bold', letterSpacing: 1 }}>
                {data.saldo}
              </Text>
            </View>
            <View style={styles.pc_container3}>
              <TouchableOpacity onPress={()=> adProduct(data)}>
                <MaterialCommunityIcons name="cart" style={{ fontSize: 12, color: COLOURS.white, padding: 5, 
                  borderRadius: 100, backgroundColor: data.selected === true ? COLOURS.red : COLOURS.green }}/>
              </TouchableOpacity>
            </View>
          <Image source={data.productImage} style={{ borderTopLeftRadius:50, borderTopRightRadius:50,
  ...
  return (
    <Observer>{()=>
      <View style={{ width:width, height: '100%', backgroundColor: COLOURS.white }}>
              ...
              {productFamily
                  ? productFamily.map((data,index)=>{
                    let opacity = possition.interpolate({
                      ...
                      extrapolate:'clamp' });
                    return(
                      <Animated.View key={index} style={styles.c_animate} opacity={opacity}/>)
                    })
                  : null}
            </View>
          </View>
          <View style={styles.c_container_5}>
            <Text style={{fontSize: 18, color: COLOURS.black, fontWeight: '400', letterSpacing: 1 }}>
              {selectedId[1]}
            </Text>
            <Text style={{fontSize: 14, color: COLOURS.black, fontWeight: '400', opacity: 0.5, marginLeft: 10 }}>
